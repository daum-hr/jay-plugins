...
새 버전이 나왔다고 안내받으시면, 채팅창에 이 한 줄만 치고 Claude Code를 다시 켜세요.

```
/plugin marketplace update jay-plugins
```

"✔ Successfully updated" 가 나오면 끝입니다.

> 혹시 `/plugin` 에서 다시 설치를 시도하셨는데 **"이미 설치돼 있어요"** 라고 나와도
> **정상이에요.** 위 명령이 이미 내용을 최신으로 바꿔놨기 때문입니다 — 실패한 게 아니니
> 그대로 Claude Code만 다시 켜시면 됩니다.
