---
name: status
description: 지금 git 저장소가 무슨 상태인지 쉬운 한국어로 알려주는 진단 도구. "지금 어떤 상태예요?" / "뭔가 꼬인 것 같아요" / "이 에러가 뭐예요?" / git 관련해서 어디서부터 시작할지 모를 때 먼저 실행. 읽기만 하고 아무것도 바꾸지 않아요. 결과에 따라 알맞은 스킬(push, pull, fix-conflict, undo, github-login)로 안내합니다.
---
...
> "지금 '기록 열람 모드'예요 — 예전 시점을 들여다보는 상태라, 여기서 저장해도 어느
> 갈래에도 붙지 않고 나중에 찾기 어려워집니다."

저장 안 된 변경이 있으면 구조 옵션을 AskUserQuestion으로 제시한다: ① 지금 작업을 새
갈래에 안전하게 담기(`git switch -c rescue/{날짜}`) ② 원래 갈래로 돌아가기(변경은 함께
따라감) ③ 그대로 두고 팀에 물어보기. 깨끗하면 ①은 빼고 제시한다.
...
```
### 지금 상태
- 지금 갈래: `feature/공지사항-화면` (main이 아니라 안전한 작업 공간이에요)
- 저장 안 된 변경: 3개 파일
- 내 컴퓨터에만 있는 저장: 2건 (아직 팀에 안 올림)
- 팀 저장소: 연결됨 (github.com/회사/저장소)
- 보관함(stash): 1개 들어 있음 — "표 정렬 작업 중"
```
...
| 관찰된 상태 | 안내 |
|---|---|
| 저장 안 된 변경 있음 | `/jay-skills:commit` — 지금까지 작업을 저장 |
| 내 컴퓨터에만 있는 저장 있음 | `/jay-skills:push` — 팀에 올리기 |
...
| 뭔가 잘못 저장한 것 같다 | `/jay-skills:undo` |
| 올린 작업을 리뷰받고 싶다 | `/jay-skills:pr` |
