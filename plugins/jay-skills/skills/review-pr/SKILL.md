...
`gh` 명령은 서버에 접속한다 — §3.6대로 **실행 전에 미리 알린다**: "PR 내용을
불러올게요 — 몇 초 걸릴 수 있어요."
...
- **`gh` 없음** → 설치 안내만 하고 끝낸다 (자동 설치 금지):
  > "PR을 보려면 `gh`라는 GitHub 도구가 필요해요. **Mac**: `brew install gh`
...
> "이건 본인이 올린 PR이에요. 승인은 동료가 해주는 게 의미가 있어요 — 내용 요약은
> 그대로 보여드릴게요."
...
> 부분을 정리하고 /jay-skills:push 하시면 이 PR이 자동으로 갱신돼요.`"
