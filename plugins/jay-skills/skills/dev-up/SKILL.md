...
`.claude/launch.json`이 이미 있으면 그 설정 그대로 쓴다. 없어서 앱이 새로 만들게 되면
**절대경로가 들어가지 않는지** 확인한다 (앱은 사용자별 경로를 적는 경향이 있는데,
그대로 커밋하면 동료 컴퓨터에서 깨진다). 띄운 뒤 아래 접속 확인부터 이어서 한다.
...
① 이면 방금 확인한 명령·포트로 만든다. **절대경로 금지**(컴퓨터마다 다르다),
`"autoPort": true` 포함, 시크릿 넣지 않기 — 커밋되는 파일이다.

**버전 관리 도구를 쓰는 프로젝트라면 그 도구를 경유하게 쓴다.** 앱이 서버를 띄울 때의
PATH가 터미널과 다를 수 있어서, 명령 이름만 적으면 **엉뚱한 버전이 잡혀 서버가 죽는다**
(실측: 같은 조건에서 `npm`은 옛 버전, 경유하면 프로젝트가 지정한 버전). Volta를 쓰는
프로젝트면 이런 모양:
...
만들었으면 git에 잡히는 새 파일이라는 것을 알린다 (커밋은 `/jay-skills:commit`).
...
- launch.json에 **절대경로와 시크릿을 넣지 않는다** — 커밋되는 파일이다. 버전 관리
  도구가 있으면 그것을 경유하게 쓴다 (PATH가 달라도 맞는 버전이 잡히도록).
